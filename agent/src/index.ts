...
async function run(): Promise<void> {
  ...
  // Run self-mod checks immediately on launch, not just on their
  // weekly cron. heartbeat.start() already runs one immediate tick,
  // but scheduler.ts's getDueTasks() only treats a never-run task as
  // due if its cron's next-run-from-24h-ago falls before now — for a
  // weekly cron that's never true on a fresh install (7 days > the 24h
  // backdate window), so check_for_updates/soul_reflection would
  // otherwise sit silent until the following Sunday. forceRun bypasses
  // the due-check entirely and runs the task directly, so this
  // guarantees both actually happen on this boot regardless. Fire-and-
  // forget: don't block startup on a git fetch or the reflection LLM
  // call, and don't let a failure here (e.g. no git remote configured)
  // stop the agent from coming up.
  for (const taskName of ["check_for_updates", "soul_reflection"]) {
    heartbeat.forceRun(taskName).catch((err) => {
      logger.error(`[SELF-MOD] Initial ${taskName} run failed`, err instanceof Error ? err : undefined);
    });
  }
